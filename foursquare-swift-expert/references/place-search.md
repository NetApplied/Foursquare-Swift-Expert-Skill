...
| radius | integer | No | Sets a radius distance (in meters) used to define an area to bias search results. The maximum allowed radius is 100,000 meters. Radius can be used in combination with ll or ip biased geolocation only. By using radius, global search results will be omitted. If not provided, default radius applied is 22000 meters. | Min: 0; Max: 100000; Format: int32 |
| fsq_category_ids | string | No | Filters the response and returns FSQ Places matching the specified categories. Supports multiple Category IDs, separated by commas. For a complete list of Foursquare Category IDs, refer to the <a href="https://docs.foursquare.com/data-products/docs/categories" target="blank">Category Taxonomy</a> page. |  |
| fsq_chain_ids | string | No | Filters the response and returns FSQ Places matching the specified chains. Supports multiple chain IDs, separated by commas. For more information on Foursquare Chain IDs, refer to the <a href="https://docs.foursquare.com/data-products/docs/chains" target="blank">Chains</a> page. |  |
| exclude_fsq_chain_ids | string | No | Filters the response and returns FSQ Places not matching any of the specified chains. Supports multiple chain IDs, separated by commas. Cannot be used in conjunction with exclude_all_chains. For more information on Foursquare Chain IDs, refer to the <a href="https://docs.foursquare.com/data-products/docs/chains" target="blank">Chains</a> page. |  |
| exclude_all_chains | boolean | No | Filters the response by only returning FSQ Places that are not known to be part of any chain. Cannot be used in conjunction with exclude_chains. |  |
...
| min_price | integer | No | Restricts results to only those places within the specified price range. Valid values range between 1 (most affordable) to 4 (most expensive), inclusive. | Min: 1; Max: 4; Format: int32 |
| max_price | integer | No | Restricts results to only those places within the specified price range. Valid values range between 1 (most affordable) to 4 (most expensive), inclusive. | Min: 1; Max: 4; Format: int32 |
...
| tel_format | string | No | Specifies the format of the returned telephone number. | Enum: NATIONAL, E164 |
...
| sort | string | No | Specifies the order in which results are listed. | Enum: RELEVANCE, RATING, DISTANCE, POPULARITY |
...
struct PlaceSearchRequest: Codable, Identifiable, Hashable {
    let query: String?
    let ll: String?
    let radius: Int?
    let fsqCategoryIds: String?
    let fsqChainIds: String?
    let excludeFsqChainIds: String?
    let excludeAllChains: Bool?
    let fields: String?
    let minPrice: Int?
    let maxPrice: Int?
    let openAt: String?
    ...
    var id: String { "place-search-request" }

    enum CodingKeys: String, CodingKey {
        case query = "query"
        case ll = "ll"
        case radius = "radius"
        case fsqCategoryIds = "fsq_category_ids"
        case fsqChainIds = "fsq_chain_ids"
        case excludeFsqChainIds = "exclude_fsq_chain_ids"
        case excludeAllChains = "exclude_all_chains"
        case fields = "fields"
        case minPrice = "min_price"
        case maxPrice = "max_price"
        case openAt = "open_at"
        case openNow = "open_now"
        case telFormat = "tel_format"
        case ne = "ne"
        case sw = "sw"
        case near = "near"
        case sort = "sort"
...
struct PlaceSearchResponse: Codable, Identifiable, Hashable {
    ...
    var id: String { "place-search-response" }
...
final class PlaceSearchClient {
    ...
    func placeSearch(parameters: PlaceSearchRequest) async throws -> PlaceSearchResponse {
        let endpointPath = "/places/search"
        ...
        if let fsqCategoryIds { queryItems.append(URLQueryItem(name: "fsq_category_ids", value: fsqCategoryIds)) }
        if let fsqChainIds { queryItems.append(URLQueryItem(name: "fsq_chain_ids", value: fsqChainIds)) }
        if let excludeFsqChainIds { queryItems.append(URLQueryItem(name: "exclude_fsq_chain_ids", value: excludeFsqChainIds)) }
        if let excludeAllChains { queryItems.append(URLQueryItem(name: "exclude_all_chains", value: String(excludeAllChains))) }
        if let fields { queryItems.append(URLQueryItem(name: "fields", value: fields)) }
        if let minPrice { queryItems.append(URLQueryItem(name: "min_price", value: String(minPrice))) }
        if let maxPrice { queryItems.append(URLQueryItem(name: "max_price", value: String(maxPrice))) }
        if let openAt { queryItems.append(URLQueryItem(name: "open_at", value: openAt)) }
        if let openNow { queryItems.append(URLQueryItem(name: "open_now", value: String(openNow))) }
        if let telFormat { queryItems.append(URLQueryItem(name: "tel_format", value: telFormat)) }
        ...
        if !(200...299).contains(httpResponse.statusCode) {
            let decodedError = try? JSONDecoder().decode(FoursquareErrorResponse.self, from: data)
                ...
                case 400:
                    throw .httpStatus(code: 400, detail: decodedError?.errorDetail)
                case 401:
                    throw .httpStatus(code: 401, detail: decodedError?.errorDetail)
                default:
