...
- Method: `POST`
- Path: `/places/{fsq_place_id}/suggest/merge`
- Operation ID: `suggest-merge`
...
| duplicate_fsq_place_ids | string | Yes | A list of string identifiers that represent the same place as the given fsq_place_id. |  |
...
struct SuggestMergeRequest: Codable, Identifiable, Hashable {
    let fsqPlaceId: String
    let dryRun: Bool?
    let duplicateFsqPlaceIds: String
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case dryRun = "dry_run"
        case duplicateFsqPlaceIds = "duplicate_fsq_place_ids"
    }
}

struct SuggestMergeResponse: Codable, Identifiable, Hashable {
    let suggestedEdits: [JSONValue]?
    let errors: [String]?

    var id: String { "suggest-merge-response" }
...
final class SuggestMergeClient {
    ...
    func suggestMerge(parameters: SuggestMergeRequest) async throws -> SuggestMergeResponse {
        let endpointPath = "/places/\(parameters.fsqPlaceId)/suggest/merge"
        ...
        queryItems.append(URLQueryItem(name: "duplicate_fsq_place_ids", value: duplicateFsqPlaceIds))
        ...
        do {
            return try JSONDecoder().decode(SuggestMergeResponse.self, from: data)
        } catch {
